...
function removeFirstOccurrences(str, value) {
  const firstMatchIndex = str.indexOf(value);
  const wordLength = value.length;
  return firstMatchIndex === -1
    ? str
    : str.slice(0, firstMatchIndex) + str.slice(firstMatchIndex + wordLength);
}
...
function removeLastOccurrences(str, value) {
  ...
  return lastMatchIndex === -1
    ? str
    : str.slice(0, lastMatchIndex) + str.slice(lastMatchIndex + wordLength);
}
...
function isPalindrome(str) {
  const cachedStr =
    str.endsWith('!') || str.endsWith('?') || str.endsWith('.')
      ? str.slice(0, str[str.length - 1])
      : str;
  return (
    cachedStr.toLowerCase().replaceAll(' ', '').split('').reverse().join('') ===
    cachedStr.toLowerCase().replaceAll(' ', '')
  );
}
...
function extractNameFromTemplate(value) {
  const words = value.slice(0, -1).split(' ');
...
function encodeToRot13(str) {
  ...
  const outputLetters = 'nopqrstuvwxyzabcdefghijklmNOPQRSTUVWXYZABCDEFGHIJKLM';
  return str.replace(
    /[a-z]/gi,
    (letter) => outputLetters[inputLetters.indexOf(letter)]
  );
}
